...
const UpdateUser = ({ userID, visible, onClose, refetch }) => {
  ...
  const handleUpdate = () => {
    ...
    const updatedStatus = formData.accountStatus === "true"; // Chuyển chuỗi thành boolean

    userApi
      .updateStatus({ id: userID, accountStatus: updatedStatus })
      .then(() => {
        Swal.fire("Cập nhật thành công!", "Trạng thái đã được cập nhật.", "success");
        refetch();
        onClose();
      })
      .catch((error) => {
